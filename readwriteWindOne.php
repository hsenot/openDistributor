<?php
    ...
    try {
        ...
        $full_url = 'http://'.$_SERVER['HTTP_HOST']."/openDistributor/getWind.php"."?". http_build_query($rec["rows"][0],'', '&');
        //echo $full_url;
        // Download the web service response
        $json = json_decode(file_get_contents($full_url));

        // Record the distributor information
        $sql = "insert into australia_grid_wind (gid,speed,the_geom) select gid,".$json->speed.",the_geom from australia_grid WHERE gid=".$p_id;
        $recordSet = $pgconn->prepare($sql);
        $recordSet->execute();        
